...
vario_fit <- 
  ...
  variogram_lm, variogram.model = 'RMexp', param = c(variance = 20000, nugget = 2000, scale = 500), 
...
#como realizar a prediçao por lm?????????????????
...
```{r}
lm_fit <- lm(argila_ ~ 1, dnos_in) #esta correto o 1??????????????????????
summary(lm_fit)
```
